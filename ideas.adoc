...
14 de Agosto.
El drag and drop funciona bien. Debería tener un panel para no arrastrar todo el nodo, pero bueno, de momento queda así.

Otro fallo que había importante era que al guardar luego no se recuperaba bien. Revisemos eso.

15 de Agosto de 2022
El guardado está reparado. Volvamos a listar las cosas pendientes más urgentes:
- La lista de notas no admite más de un espacio seguido
- Las notas no pueden superar el 12
- Hay algunas cosas que sobran, como el pulse y otras.
Vamos con ello.

Bien, todo arreglado, las notas pueden incluir espacios, el número de nota puede ser cualquiera ( aunque solo entre 83 y -83, hay que atrapar el exceso aquí)  y se limpiaron las cosas.

Habría que pensar de nuevo la dirección correcta del proyecto:
- Debería poder recargarse la canción sin perder el ritmo
- Debería poder tocarse solamente una parte (ahora está el botón pero no funciona)
- Nombres de las escalas
- Carga de objetos e inyección de dependencias
- Sintetizadores adicionales
- Percusión

Vale, arreglado lo de tocar una parte.
- Debería poder recargarse la canción sin perder el ritmo
- Nombres de las escalas
- Carga de objetos e inyección de dependencias
- Sintetizadores adicionales
...
Quizá un botón nuevo para recargar la canción, de manera que solo empiece con los nuevos datos cuando haya terminado con los actuales. Aunque quizá sea incómodo, esperar a que termine toda la canción. Igual solamente se necesita que no pierda el ritmo.
Bueno, esto se puede dejar como una sofiscación futura.
Vamos con algo más urgente:

Vale, una cosa práctica sería lo de mostrar los nombres de los elementos en lugar de números, por ejemplo, las escalas, o los playmodes. Estoy mirando bien cómo va la directiva ngSwitch y parece que se puede hacer.
Hay que intentar no salirse del objetivo principal: este debe ser solo una herramienta para demostrar la teoría de la música de partículas, así que lo primero ha de ser la armonía y sus posibilidades.

Vale. Arreglado. Ahora se muestran los nombres de las escalas y los playmodes, lo más urgente. 

Bien, lo más importante sigue siendo el asunto armónico. Así que deberíamos pensar en mejorar ese modo random que parece tan mecánico para hacerlo más humano.
De modo que vamos a congelar de momento todo el resto de ideas y vamos a centrarnos en la parte "generativa" del proyecto. Debe generar música muy interesante, y debe ser muy fácil de usar. Vamos con el modo random.

Para empezar, no parece que sea un modo más de cómo se toca un arpegio. Principalmente porque posiblemente requiera más de un parámetro, como la cantidad de aleatoriedad, para empezar. Igual hay que pensar en algo distinto.
Puede ser más como el sistema de shift, que tiene un comienzo, un tamaño y un valor. Sería un comando como ese, donde se dijera desde dónde hasta empezar, qué tamaño de notas y qué cantidad de aleatoriedad.
Con valores bajos de aleatoriedad, funcionaría como hasta ahora, es decir, dando solamente las notas del arpegio. Con un poco de aleatoriedad podría incluir notas extra entre ellas. No tengo claro esto.
Veamos, elementos que influyen en una melodía:
- Lo lejos o cerca que está cada nota de la nota base armónicamente. Eso se mide en intervalos de tercera. 
- Si tocas las notas más lejanas en los tiempos fuertes o las usas como notas de paso para las notas más cercanas.
- Lo frecuentemente que cambias de octava
- Lo largos que pueden ser los saltos que das entre notas


También se podría usar otra técnica:
Cada nota del arpegio es una nota "central". Alrededor de las notas del arpegio se pueden agregar otras notas que actuan como puentes entre ellas. Cuanto más lejos estén de las notas centrales más aleatorio parecerá.
Por ejemplo. Si las notas son C, E, G, B podríamos hacer simplemente el arpegio o agregar entre ellas algunas notas puente, por ejemplo:
    C   E   G   B
    C D E F G A B
    C G E B G D B 
    C B E D G F B
...
Tres conceptos interesantes:
    - distancia armónica: Distancia medida subiendo por terceras
    - distancia interválica: Distancia a una nota subiendo por grados
    - distancia octávica: Distancia a una nota subiendo por octavas

Por ejemplo, para pasar de C a E hemos incluido en medio varias opciones:
    - Si usamos un D tenemos:
        - Distancia armónica: 4 (C-> E-> G-> B-> D)
        - Distancia interválica: 1 (C-> D)
        - Distancia octávica: 0 (C-> D) (Si tomamos el D de la misma octava)
    - Si usamos un G tenemos:
        - Distancia armónica: 2 (C-> E-> G)
        - Distancia interválica: 4 (C -> D ->E ->F ->G)
        - Distancia octávica: 1 (C-> G) (Si tomamos el G de la misma octava)        
    - Si usamos un B tenemos:
        - Distancia armónica: 3 (C-> E ->G ->B)
        - Distancia interválica: 6 (C-> D-> E-> F-> G-> A ->B)
        - Distancia octávica: -1 (B<- C) (Si tomamos el B de la octava anterior)        

Creo que la distancia "octávica" sobra por completo. Realmente las otras dos se manejan mejor solas.
Si seleccionas una baja distancia interválica, la nota se tomará de la octava donde quede más cerca de la nota base, automáticamente. Si tomas una alta distancia interválica, pueden aparecer saltos de dos o tres octavas perfectamente. Bien.
La distancia armónica marcará frases más melódicas, si dejamos que se vaya muy lejos de la base o más armónicas, si nos reducimos al arpegio básico.
A las distancias seleccionadas, habrá que agregarle el desplazamiento de la nota objetivo, aquella a la que queremos "decorar", agregándole notas antes o después. De manera que si en la nota anterior agregamos una segunda por encima y mantenemos las mismas distancias, en la siguiente nota obtendremos una segunda por encima de dicha nota, no la de la anterior. Así se mantiene la naturalidad melódica.
En cuanto al tiempo, se podría hacer que para agregar notas, la nota "adornada" o su anterior se dividieran para formar más notas, de manera que la medida general no cambie.
La división rítmica que se genera también puede ser parametrizada mediante una "distancia métrica", es decir, una distancia que se toma como referencia para la división rítmica. Por ejemplo, con una baja distancia métrica, de una negra pasaremos a dos corcheas, por ejemplo, pero con una más alta podríamos pasar a cuatro semicorcheas, o incluso a tresillos. Incluso se podrían generar silencios, aunque igual esta sería otra distancia, la distancia "sónica". A más distancia sónica, más silencios.
Resumen de momento:
    -Distancia armónica: más o menos lejos de la base en el arpegio
    -Distancia interválica: más o menos lejos de la base en la escala
    -Distancia métrica: más o menos lejos de la medida original
    -Distancia sónica: más o menos lejos de la cantidad de silencio

Creo que el adorno se aplica sobre la nota actual. No es una nota de paso entre dos, sino una sustitución de una nota por un conjunto de notas.
Los parámetros podrían ser entonces:
    - Inicio: primera nota del arpegio que se adorna
    - Duración: durante cuántas notas se hace el adorno
    - Distancia armónica: lo lejos en el arpegio que se puede llegar
    - Distancia interválica: lo lejos en la escala que se puede llegar
    - Distancia métrica: lo granular que puede ser la nueva medida
    - Distancia sónica: la cantidad de silencios generados

La distancia interválica puede medirse desde la nota central en notas hacia arriba y hacia abajo. Un valor de cero indicaría toda la octava central, es decir, la octava que rodea a la nota decorada. Por ejemplo, si tenemos  que decorar un C sería:
    G A B (C) D E F
El valor 0 indicaría que las notas generadas han de caer dentro de esa octava. A partir de ahí, cada número iría agregando una nota, tanto por arriba como por abajo, así que un 1 indicaría:
    F G A B (C) D E F G
Y un 2 indicaría
    E F G A B (C) D E F G A
Etc... De esta forma limitamos más o menos lo lejos que pueden estar las notas de la nota base, tanto ascendente como descendentemente.
La distancia armónica nos serviría para elegir entre esas notas las más cercanas o lejanas en el arpegio.

Resumen de la distancia interválica:
    0:     (G A B (C) D E F)      (solo en la octava central)
    1:   (F G A B (C) D E F G)    (1 nota arriba y abajo)
    2: (E F G A B (C) D E F G A)  (2 notas arriba y abajo)
    etc...
(Horas mas tarde...)
Estoy pensando que esto de que 0 en la distancia interválica signifique toda la octava central no parece muy práctico. Cero debería significar cero, nada, ninguna nota. No se admite ninguna más que la propia nota central, porque ninguna estará en la lista. Si ponemos uno sería admitir solo una nota por delante y otra por detras entonces se admitirían esas dos. Más simple de calcular. Resumiendo:
Resumen de la distancia interválica:
    0:           ((C))            (solo en la nota central)
    1:         (B (C) D)          (dos notas)
    2:       (A B (C) D E)        (cuatro notas)
    3:     (G A B (C) D E F)      (seis notas)
    4:   (F G A B (C) D E F G)    (ocho notas)
    5: (E F G A B (C) D E F G A)  (diez notas)
    etc...

Una distancia armónica de 0 podría indicar solamente el arpegio base, es decir 1, 3 y 5. A partir de ahí, cada número iría agregando una nota del arpegio, por ejemplo:
0: C E G
1: C E G B
2: C E G B D
3: C E G B D F
...
No. Quizá mejor empezar desde cero para una distancia nula, que no generaría ninguna nota:

0: No se agrega nota alguna
1: C
2: C E
3: C E G
4: C E G B
...
7: C E G B D F A
...
(Para las escalas de más de 7 notas esto continuaría agregando notas del arpegio)

La distancia rítmica dividirá la nota original en una cantidad de notas igual a la distancia rítmica. Por ejemplo:
0: No se divide la nota original
1: Se divide en dos notas
2: Se divide en tres notas
3: Se divide en cuatro notas
4: Se divide en cinco notas
5: Se divide en seis notas
6: Se divide en siete notas
7: Se divide en ocho notas
etc...
O sea, la distancia es la CANTIDAD de notas que se agregan a la original dividiendo el espacio para que siga siendo el mismo.
Todos estos parámetros pueden ser fijos o RANDOM.
...
Los valores siempre son un random entre cero y el valor seleccionado.

A las distancias les llamaremos por sus iniciales: DA, DI, DM, DS.
    - DA: 4 (random entre 0 y 4) -> (C E G B D)
    - DI: 1 (random entre 0 y 1) -> (F G A B (C) D E F G) 

Mejor las siglas en inglés: AD, ID, MD y SD.
Incluso, mejor que "distance" usaremos "gap", que es más corto y expresa lo mismo:
AG, IG, MG y SG. 

Aquí, por ejemplo, si la nota generada es un D tiene que ser el de la misma octava, es decir, el que está justo encima de C, porque a eso nos obliga la DI. Sin embargo, si sale un F puede ser el superior o el inferior, puesto que la DI incluye a ambos.

La distancia sónica indica lo probable que es que una nota sea sustituida por un silencio.
0: ninguna nota se silencia
1: un 10% de las notas se silenciarán
8: un 80% de las notas se silenciarán
etc
10: todas las notas se silenciarán

IMPORTANTE: Con este sistema, estamos controlando solamente las notas que se AGREGAN a la nota original. En ningún momento la nota original se va a sustituir o eliminar. Quizá esto deba controlarse también.

Vale, teníamos un parámetro que era el INICIO, que indica desde qué nota se hace el adorno. Si ese inicio es 0 entonces la nota original entra a formar parte del cambio, con lo que podrá ser sustituida. Si es 1 entonces no, solamente se agregan notas. Esto puede ser práctico.
Entonces, el parámetro que no tiene sentido es la duración, puesto que siempre será la de la nota original. Entonces resumen:
...
Vale. Creo que tenemos una gran idea. Vamos a pensar cómo llevar esto a la práctica. Nueva rama!

Bien, tenemos la infraestructura y los parámetros listos. Ahora pensemos el algoritmo.

La función es
getDecoratedNotes(notes:number[], armonicGap:number, intervalicGap:number, metricGap:number, sonicGap:number):number[]

Recibe las notas y los parámetros que necesita. Dividamos el trabajo en partes:
-Obtener el conjunto de notas del armonicGap
-Obtener el mapa de notas del intervalicGap con su posición respecto a la nota base que es 0.
-Obtener la cantidad de notas indicadas por el metricGap
-Obtener la nueva duración de las notas que se van a generar
-Para cada nota:
    - Elegir una nota del conjunto armónico
    - Si está dentro del conjunto interválico elegir su posición
    - Si no está generar un silencio
    - Si hay varias elegir una aleatoria

Vale. Creo que se puede optimizar:
Cuando elegimos el conjunto armónico puede contener más notas que las del conjunto interválico. Así que podrían generarse notas que no acabarían agregándose debido a que no entran en el filtro. Luego será mejor empezar por filtrar el conjunto armónico y dejar solamente las notas POSIBLES.

Ejemplo de esto:
    Conjunto armónico: [C E G B D]
    Conjunto interválico: [A B (C) D E]

Claramente, la nota G no puede estar, porque el conjunto interválico no incluye la nota.
Luego primeramente se filtra:
    Conjunto armónico: [C E B D]
    Conjunto interválico: [A=-2 B=-1 (C=0) D=1 E=2]    
Ahora se seleccionan las notas sabiendo que todas pueden entrar.
Si ocurre al revés, que el armónico incluye menos que el interválico:
    Conjunto armónico: [C E]
    Conjunto interválico: [A=-2 B=-1 (C=0) D=1 E=2]    
Simplemente el filtro lo deja igual, así que todas las notas entrarán.
Si tenemos varias ocurrencias de cada nota:
    Conjunto armónico: [C E G B D]
    Conjunto interválico: [F=-4 G=-3 A=-2 B=-1 (C=0) D=1 E=2 F=3 G=4]
En este caso, si seleccionamos el G encontraremos dos ocurrencias: -3 y 4. En ese caso se selecciona una aleatoria.
Entonces necesitamos una función que haga el filtro, y también una que si hay más de una ocurrencia, elija una al azar.
Entonces, nuevo resumen:

-Obtener el  armonic set
-Obtener el intervalic map (con su posición respecto a la nota base que es 0)
-Obtener el filtered intervalic set
-Obtener la cantidad de notas indicadas por el metricGap
-Obtener la nueva duración de las notas que se van a generar
-Para cada nota:
    - Elegir una nota al azar del filtered intervalic set
    - Obtener el conjunto de posiciones de esa nota en el intervalic set
    - Si son más de una elegir una al azar

Una vez que tenemos todas las notas las devolvemos con la duración asignada

Para obtener la duración de las notas hay que usar el metricGap, que nos indica la probabilidad de que una nota sea sustituida por una cantidad mayor o menor de notas. Con un metricGap de 0, no se agregará ninguna nota, con 1 se agregará una, así que tendremos la nota original más una nota. Obviamente la suma de la duración de ambas será lo que duraba la original. Si se agregan 2 tendremos un tresillo, si se agregan 3 tendremos 4 notas, etc.
Esta duración, en principio se reparte igualmente entre todas las notas. Pero más adelante puede aleatorizarse también.

Todo esto se hace para cada nota del arpegio que se recibe
-Obtener el  armonic set [D F A C E G]
-Obtener el intervalic map (con su posición respecto a la nota base que es 0) [G=-4 A=-3 B=-2 C=-1 (D=0) E=1 F=2 G=3 A=4]
-Obtener el filtered intervalic set [D F A C E G]
-Obtener la cantidad de notas indicadas por el metricGap (3)
-Obtener la nueva duración de las notas que se van a generar (tresillo)
-Para cada nota:
    - Elegir una nota al azar del filtered intervalic set (G)
    - Obtener el conjunto de posiciones de esa nota en el intervalic set [3, -4]
    - Si son más de una elegir una al azar [3]

Vale el algoritmo que toma la distancia interválica y obtiene las notas en cada desplazamiento debería:
Dado un desplazamiento, positivo o negativo, devolver el grado de la escala en esa posición, por ejemplo:
    Escala: [C D E F G A B]
Desplazamiento   Grado
0               C
1               D
2               E
-1              B
...
-8              B
etc    
Vale, si el grado es positivo le hacemos el módulo sobre el tamaño de la escala
Si es negativo le hacemos el módulo sobre el tamaño de la escala y le sumamos el tamaño de la escala

Aclaración:
ArmonicSet:  Posiciones del arpegio que el armonicGap permite. Sería el "with" del asunto
IntervalicMap: Mapa con los GRADOS de la escala y en qué posiciones se encuentran dado ese gap
FilteredIntervalicSet: Eliminamos los GRADOS CUYA POSICIÓN en la escala no está en el armonicSet

18 de Agosto.
Las cosas estan revueltas en mi mente, no consigo que funcione y la razón es la confusión entre notas, intervalos, desplazamientos, grados, etc.
Vamos a intentar aclarar estos conceptos antes de seguir.

Una escala tiene un patrón de grados, es decir, la cantidad de semitonos a los que está cada nota desde la nota base. Por ejemplo, la escala WHITE sería:

    0 2 3 5 7 9 10

Si estamos en el tono de D (es decir, C mayor) las notas resultantes serían:

    0 2 3 5 7 9 10
    D E F G A B C 

Hay una función gradeToSemitones que, dado un grado, devuelve la cantidad de semitonos que hay entre ese grado y la nota base.
Por ejemplo, dado el grado 28 o el -19, nos devuelve la cantidad de semitonos que hay entre ese grado y la nota base.
Los grados son grados de la escala, es decir, la 28ava nota o la tercera o la novena nota.

Hay otro getSelectedNotes que, dado una nota raíz, una densidad, una tonalidad y un gap, nos devuelve el conjunto de notas que se generarán. Comenzando desde esa raíz, con ese gap de separación, esa densidad de notas y en esa tonalidad.

Hay también un getNotePosition que, dado un número de nota, nos devuelve la posición en la escala de ese número. No sé si está bien esa función.

Luego está la importnate gradeToChord, que dado un grado, nos devuelve el acorde que se generará en ese grado.
El problema es que el acorde ya se devuelve en semitonos, es decir, ya calculados los semitonos correspondientes a las notas de la escala en las octavas correspondientes. Así que puede devolvernos valores como 45 o 110 o -54. Entonces el problema es que necesitamos variar las notas antes de que se conviertan en semitonos para poder variarlas dentro de la escala correcta pero también necesitamos que sean las notas finales para poder variar las notas generadas. Entonces, cómo lo hacen??

Igual no podemos esperar a que ocurra, hay que intervenir MIENTRAS está ocurriendo, me explico:
Dentro de la función getSelectedNotes, se generan las notas que esa densidad y ese gap han seleccionado. Es decir, las notas del arpegio.
Esto parece más complejo. 
La posibilidad de hacerlo a posteriori es mínima, pero se puede analizar.
Tendremos las notas ya como semitonos, en sus octavas correspondientes. A cada nota tendremos que agregarle los complementos generados en esa misma octava. Podríamos primero convertir la nota a grado y a octava y luego operar con esos valores. Quizá sea más práctico.

O sea, tomo una nota, que es una cantidad de semitonos desde la base, la convierto a grado y a octava. Genero las notas de complemento para esa octava. Luego a las notas generadas les sumo esa octava y debería funcionar. Vamos a verlo.


Tengo todavía alguna confusión con los grados de la escala:
Una cosa es la posición de la escala, que contiene una cantidad de semitonos. Bien.
Otra cosa es un shift, que es el desplazamiento desde la posición cero, que puede ser negativo.
Obviamente, no es una posición del array, porque no hay negativos, pero puede convertirse mediante la función adecuada.
Así que habrá una función para convertir positionShift en arrayPositions. Eso es necesario.

Viéndolo así, entonces intervalicSet es un Map<grade, shift[]>, es decir, en que shift estará el grado.
Lo estábamos haciendo mal.

Vale, la clase Scale necesita un api claro y eficiente. Vamos con ello:

La clase Scale define un patrón de desplazamientos que forman una escala musical. 
Los semitonos que definimos son solamente los que entran en una octava. El resto se calculan. Pueden ser positivos y también negativos.
Se calculan siguiendo el patrón de la escala correspondiente y haciendo % con el tamano de la escala.

Es sencillo saber qué semitonos hay en una posición de la escala, es simplemente el valor que guarda esa posición. Para saber en qué posición está un semitono (es decir, qué grado de la escala es) habrá que hacer la operación contraria: indexOf(semitono).

Si no todos los semitonos pertenecen a una escala, entonces esta función puede devolver error. Además, un semitono puede caer en una octava diferente de la inicial, así que para pasar desde un semitono a una nota devolveremos un error o bien una nota y además una octava.

    - Función que devuelve el grado en la escala de un semitono dado ( positivo, negativo, incluso fuera de la octava). Devuelve el grado de la nota y la octava en la que cae, o bien undefined si no existe ese semitono en la escala.
        semitoneToGrade(semitone:number):{grade:number, octave:number}| undefined
        Ejemplo: 
            semitoneToGrade(0) devuelve {grade:0, octave:0}
            semitoneToGrade(2) devuelve {grade:1, octave:0}
            semitoneToGrade(7) devuelve {grade:4, octave:0}
            semitoneToGrade(12) devuelve {grade:0, octave:1}
            semitoneToGrade(-1) devuelve {grade:6, octave:-1}

    - Función que devuelve el semitono en la escala de un grado y una octava. Devuelve el semitono de la nota y la octava en la que cae, o bien undefined si no existe ese grado en la escala. El grado parámetro puede ser negativo, o mayor que el tamaño de la escala.
    El semitono devuelto y la octava siempre existirán, no es posible que devuelva undefined.        
        gradeToSemitone(grade:number, octave:number):semitone:number
        Ejemplo: 
            gradeToSemitone(0, 0) devuelve 0
            gradeToSemitone(1, 0) devuelve 2
            gradeToSemitone(4, 0) devuelve 7
            gradeToSemitone(0, 1) devuelve 12
            gradeToSemitone(6, -1) devuelve -1

    Podemos cambiar los nombres para usar notas y grados. Las notas son los semitonos, y los grados las posiciones del array. Las posiciones son "virtuales" respecto a que pueden estar fuera del array pero porque son de otra octava, pero el número es el mismo.
    Así que con los nuevos nombres quedarían:

    noteToGrade(note:number):{grade:number, octave:number}| undefined                
    gradeTonote(grade:number, octave:number):note:number

Respecto a intervalicGap y a armonicGap, por cierto, vamos a cambiar intervalicGap a melodicGap.
Esos dos elementos no están muy claros ya. Volvamos sobre ellos.
Los dos deben indicar rangos de grados, no de notas. Uno de ellos indica rangos positivos y negativos, de forma simétrica. El otro indica rangos de grados alternos, que forman los acordes (aunque esto podría parametrizarse también)
Vale, entonces tendremos que hacer cálculos con estos elementos. Por ejemplo:

    - Dado un grado, está incluido en el melodicGap?
    - Dado un grado, está incluido en el armonicGap?

La cuestión es que en una misma nota puede estar repetida en varios grados distintos, en distintas octavas.

A ver, la razón de todo esto es que podamos elegir notas diferentes entre las posibles. Las posibles son las notas del arpegio de esa nota, es decir, de los grados alternos desde ella hacia arriba y que también podamos elegirlas de la octava anterior, debido al melodicGap.
En realidad se trata de elegir una nota del arpegio DE LA NOTA BASE para cambiarla por la nota actual. Sería como desordenar el arpegio. Lo bueno es que se puede ampliar el arpegio de notas para sustituir sin cambiar el original. La cosa es que no hay que hacerlo siempre subiendo octavas, sino que se pueden usar las notas contiguas equivalentes. Eso mide el melodicGap.
MelodicGap podría medir la probabilidad de que las notas generadas del arpegio se vuelvan a la misma octava. Digamos a una posición cerrada. Sería más simple.

Entonces cambiamos el plan: 
-Seleccionar una nota del arpegio cuyo tamaño está definido por armonicGap.
-Poner esa nota en la misma octava que la nota a la que complementa, basado en el valor de melodicGap.

Los dos parámetros pueden pasar a llamarse harmony y melody. Más simple.
La cantidad de harmony medirá lo que se puede ampliar el arpegio de complemento. El parámetro melody medirá lo probable es que cada nota permanezca en la misma octava que la anterior, o incluso mejor, que tome la octava en la que esté más cerca intervalicamente de la nota a la que complementa. Bien.

Esto parece realmente simple, ahora hay que ver cómo implantarlo.

Una observación, el gap con el que se expande el parámetro harmony ha de tomarse del gap del arpegio inicial, porque si no coinciden no afinaremos.

Otra observación: si una nota está separada por otra más de tres grados, entonces queda más cerca la de la octava contraria. Esto nos puede servir para elegir la nota que se va a sustituir.

Entonces, acciones que se necesitan:
    - Dada una BaseWidth y un BaseGap elegir la nota siguiente del arpegio
    - Dada una HarmonycWidth y el mismo BaseGap, elegir una nota al azar de ese arpegio
    - En caso de seleccionarse, desplazarse de una nota a otra siempre por el camino más cercano.

21 de Agosto. Volvemos a simplificar.
Hemos decidido poner un pattern y un gap para decorar las notas del arpegio. Parece lo más simple y flexible.
Hay que probar las posibilidades que tiene.

Idea: cada vez estoy más tentado de dejar el interfaz gráfico de lado por completo y hacer un lenguage.
Podría tener la sintaxis de YAML y eso sí que sería flexible. Hagamos que esto funcione, aunque sea sencillo y nos servirá como POC para crear el lenguaje.


Vale, interpretando el decorationPatter:
- Se hace lo mismo que para interpretar las notas de un bloque. Con eso obtenemos las "rootNotes, que, en realidad, son los rootGrades, o los grados a partir de la nota base. Bien.

Pero resulta que no es exactamente lo mismo, porque aquí podemos tener varios patrones separados por comas. Qué pena tener que hacer otra gramática... Hum... Pensemos esto.

Vale, getDecoratedGrades no está funcionando. Resumen de lo que tiene que hacer:
   getDecoratedGrades(arpegioGrades: OctavedGrade[], baseGap:number, decorationPattern: string, decorationGap?: number): OctavedGrade[]

Recibe los grados del arpegio. Para cada grado tendrá que aplicar las notas del pattern, separadas por el decorationGap. O sea. Si tenemos:
arpegio: 0 2 4 6
decorationPattern:    0 -1 3
Tendríamos que obtener:
    result: 0+(0 -1 3) 2+(0 -1 3) 4+(0 -1 -3) 6+(0 -1 3)
    o sea:    (0 -1 3)   (2 1 5 )   (4  3  2)   (6  5 9)
Si consideramos al decorationGap como multiplicador entonces:
    con un decorationGap de 2:
    result: 0+(0 -1 3)*2 2+(0 -1 3)*2 4+(0 -1 -3)*2 6+(0 -1 3)*2
    o sea:    (0 -2 6)     (4  2 10)    (8  6  2)     (12 10 18)
O sea, el pattern no representa la cantidad de GRADOS que nos desplazamos, si no la cantidad de decorationGaps que saltamos.


 Día 23 de Agosto.
 Parece que funciona. Sin embargo, en el tema de la medida estamos bloqueados. Hasta ahora, en todos los modos distintos de CHORD, el tiempo del arpegio se calculaba automáticamente. Se dividía el tiempo total entre las notas del arpegio. Porque no teníamos ninguna indicación de medida. Ok. Pero ahora tenemos un pattern de notas que pueden llevar su medida, así que deberíamos poder hacer caso de esto.
 Igual deberíamos agregar otro play mode: PATTERNS. Al usar este modo tendríamos que aceptar el tiempo de las notas de los patterns y repartir el tiempo total. Podría funcionar.
 Revisemos esto con calma.

 Bien, resulta que NO.
 Resulta que el supuesto modo "patterns" haría uso de los arpegios para montar los patrones, y los arpegios pueden elegirse en el play mode. Así que habrá que hacerlo de forma automática. Es decir, si hay patrones, entonces el tiempo se divide según el tiempo marcado por los propios patrones. Así sería más lógico.
 Bien, aunque se agregó la capacidad de guardar el tiempo en las notas de los arpegios, parece altamente complicado distribuir el tiempo correctamente. Mejor lo dejamos como estaba e intentamos progresar por otro flanco.

 Vale, en total no estoy muy contento con esta rama. Por una parte se hicieron algunas mejoras, pero la idea de los patrones no parece tan interesante como creía. Aunque igual no la he probado en profundidad. El tema de no tener las medidas controladas me preocupa un poco.
 Si embargo, ahora trabajamos con grados hasta el último momento, en que se convierten en notas. Eso ha estado bien, y abre la posibilidad de hacer más operaciones.
 Además se han agregado algunos tests. Eso también está bien.
 
 30-Marzo-2025.
 El aspecto ha mejorado ostensiblemente. Ahora deberíamos estudiar el tema de las operaciones más a fondo.
 De momento, solo tenemos incremento, decremento y asignación, y además las variables se pueden asignar solamente a elementos de las operaciones. Básicamente, solo operamos con las variables numéricas.
...
A los números habría que poder hacerles más cosas, como sumarle o restarle una cantidad concreta, o incluso multiplicarlos o dividirlos.
A las escalas se las podría incrementar o decrementar, como si fuesen valores numéricos, posiciones en la lista de escalas
A los playModes lo mismo
A las melodías ya puede ser más complejo. Veamos:
Una melodía es una lista de números, de momento, que pueden tener algunos símbolos más para indicar su duración.
También pueden incluir silencios. Habría que crear una notación más avanzada, que permitiese editar las notas y su duración e intensidad de forma gráfica
Pero además una melodía debería poder estar compuesta de variables también, es decir, cada nota, cada duración, cada intensidad debe poder ser una variable.
